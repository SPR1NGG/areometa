...
const PopupEdit = ({ setActive, showroom }: Props) => {
	const {
		register,
		handleSubmit,
		formState: { errors },
	...
	const [old, setOld] = useState<string>(
		showroom.url || `${BASE_URL}/uploads/${showroom.filename}`,
	);
	...
	const onSubmit = async (data: Inputs) => {
		...
		let uploadedVideo: AxiosResponse<string[]> | undefined;

		if (video[0]) {
			uploadedVideo = await ShowroomService.uploadVideo(video[0]);
		}

		await toast.promise(
			ShowroomService.update(
				{
					name: data.name,
					url: data.url,
					filename: uploadedVideo && uploadedVideo.data[0],
				},
				showroom.id,
			),
			{ success: 'Шоурум сохранён', pending: 'Сохранение шоурума' },
		);
			...
			>
				...
				<div>
					...
					<Input
						...
						disabled={
							video.length === 1 || (video.length === 0 && old.includes(`${BASE_URL}/uploads/`))
						}
						defaultValue={showroom.url}
						{...register('url')}
					/>
				</div>

				<ContentPreview old={old} setOld={setOld} setVideo={setVideo} file={video[0]} />
